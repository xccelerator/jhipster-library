...
type RestOf<T extends IBook | NewBook> = Omit<T, 'publishYear'> & {
...
export class BookService {
  ...
  protected convertDateFromClient<T extends IBook | NewBook | PartialUpdateBook>(book: T): RestOf<T> {
    return {
      ...book,
      publishYear: book.publishYear?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restBook: RestBook): IBook {
    return {
      ...restBook,
      publishYear: restBook.publishYear ? dayjs(restBook.publishYear) : undefined,
    };
  }
